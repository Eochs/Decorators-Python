...
class memoized(object):
    dod = { } # dictionary of mappings of functions to arguments to return val's
...
"""
        self.__name__ = f.__name__
        self.__f = f
        #dod[self.__name__] = {} # new dictionary for holding values of args
...
for later use.
""" 
      fname = self.__name__ # function name that serves as a key
      dod[fname] = {} # new dictionary for holding values of args
      key = (str(args), str(dargs)) # create unique key from arguments to func
      if self.dod[fname].has_key(key) == True:
        return self.dod[fname][key] # return result of function on those args
      else: # key not found
        result = ''
        if len(args) > 0:
          result = self.__f(*args)
        elif len(dargs) > 0:
          result = self.__f(**dargs)
        # set the args at that function to map to out
        self.dod[fname][key] = result
        return result
